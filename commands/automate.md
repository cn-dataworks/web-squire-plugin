...
argument-hint: <workflow-name> [prompt] [playwright|chrome] [headed|headless] [vision]
---

# Automate

Run a saved browser automation workflow from the project's `commands/` or `workflows/` directory with configurable defaults and overrides.
...
- **SKILL:** keyword detection — `playwright-cli` (default) or `chrome-mcp`
...
1. If no `$ARGUMENTS` provided, list all available workflow `.md` files in the project and stop.
2. Extract WORKFLOW from the first argument.
3. Use Glob to check that a `{WORKFLOW}.md` file exists in `commands/` or `workflows/`. If not found, list available workflows and stop with an error message.
4. Parse remaining arguments for keywords (SKILL, MODE, VISION) and collect leftover text as PROMPT.

### Phase 2: Load Workflow

5. Read the workflow `.md` file.
...
8. Execute the resolved skill (`/playwright-cli` or `/chrome-mcp`) with a combined prompt:
